...
def lenet(x):
    ...
    # when mnist training , shape=[5,5,3, blabla ] -> shape = [5,5,1, blabla]
    # L1: Convolutional ~ (32, 32, 1) -> (28, 28, 6)
    conv1_w = tf.Variable(tf.truncated_normal(shape=[5, 5, 3, layer_depth.get('L1')], mean=mu, stddev=sigma), name='conv1_w')
...
def train_step(loss):
    optimizer = tf.train.RMSPropOptimizer(1e-3) #이전의 optimizer 와의 차이를 알아야할듯
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        for i in range(n_batches):
            ...
            sess.run(train_step, feed_dict={x: X_, t: Y_})
    ...
    val_acc_t = accuracy.eval(session=sess, feed_dict={x: X_test, t: y_test_one_hot.eval(session=sess)})
    print()
