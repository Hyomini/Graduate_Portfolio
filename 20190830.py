...
label_x = np.array(sess.run(tf.argmax(y, 1), feed_dict={x: X_train, keep_prob: 1.0}))    # classified label of data x[i]
...
# Dimension reduction -> 2-means clustering -> split label
temp = [None] * k * num_of_labels
for label in range(k * num_of_labels):
    temp[label] = list()
for label in range(num_of_labels):
    t_sne_data = TSNE(learning_rate=300).fit_transform(data[label])
    data_frame = pd.DataFrame(t_sne_data, columns=('x', 'y'))
    data_points = data_frame.values
    k_means = KMeans(n_clusters=2).fit(data_points)
    data_frame['cluster_id'] = k_means.labels_
    ...
    for i in range(len(data[label])):
        temp[k * label + int(data_frame[cnt][2])].append(data[label][i])
        cnt = cnt + 1
data2 = np.array(temp)
