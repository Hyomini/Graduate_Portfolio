...
def loss2(cov, y, t):
    return tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=y, labels=t)) + tf.reduce_sum(cov - np.eye(64))
...
plt.imshow(np.reshape(img, [32, 32]), cmap='Greys')
plt.show()

# Mahalanobis distance classifier ~ 가장 가까운 distance를 가지는 클래스로 분류하고 ood_distance와 비교
img_f_x = sess.run(fullc2, feed_dict={x: img, keep_prob: 1.0})

a = [None]*10
for i in range(10):
    a[i] = list()
    u = np.reshape(img_f_x, (1, num_neurons))
    v = np.reshape(mu_hat[i], (1, num_neurons))
    a[i].append(distance.mahalanobis(u, v, np.linalg.inv(sigma_hat)) ** 2)
...
if class_distance[i] >= m_dist_threshold[i]:
    print('The image is classified to \'OOD\' by Mahalanobis distance classifier', end='\n')
else:
    print('The image is classified to \'' + str(int(i)) + '\' by Mahalanobis distance classifier', end='\n')
print("===============================================================================================================")
